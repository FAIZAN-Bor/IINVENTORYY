...
export default function SupplierLedgerPage() {
    ...
    const calculateRunningBalance = () => {
        let initialBalance = supplier?.openingBalance || 0;
        ...
        if (fromDate) {
            ...
            priorTransactions.forEach(txn => {
                if (txn.type === 'purchase') {
                    initialBalance += (txn.amount || 0); // Purchase increases liability (Credit)
                } else if (txn.type === 'payment') {
                    initialBalance -= (txn.amount || 0); // Payment decreases liability (Debit)
                }
            });
        }
        ...
        filteredTransactions.forEach(txn => {
            ...
            if (txn.type === 'purchase') {
                credit = txn.amount || 0; // Purchase is Credit
                runningBalance += credit;
                description = 'Purchase Bill';
            } else if (txn.type === 'payment') {
                debit = txn.amount || 0; // Payment is Debit
                        ...
                        <div className="flex flex-col items-center gap-1 mb-2 text-sm font-serif">
                            <div className="flex gap-8">
                                <div>
                                    ...
                                    <span className="border-b border-black inline-block w-20 text-center ml-2">{supplier.partyNumber || '-'}</span>
                                </div>
                                <div>
                                    <span className="font-bold">Name</span>
                                    <span className="border-b border-black inline-block min-w-[200px] text-center ml-2">{supplier.name}</span>
                                </div>
                            </div>
                            <div className="flex gap-8">
                                <div>
